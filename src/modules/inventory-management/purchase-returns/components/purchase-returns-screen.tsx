...
export function PurchaseReturnsScreen() {
  ...
  const columnDefs = useMemo((): Array<DataTableColumn<PurchaseReturn>> => {
    return [
      {
        id: "document_number",
        header: "Number",
        ...
        cell: (row) => (
          <RecordLink href={`/purchase-returns/${row.id}`}>
            {purchaseReturnDisplayNumber(row) ?? "—"}
          </RecordLink>
        ),
      },
      {
        id: "reason",
        header: "Reason",
        cell: (row) => PURCHASE_RETURN_REASON_LABELS[row.reason_code],
      },
      {
        id: "document_date",
        header: "Date",
      ...
      ...auditTimestampColumns<PurchaseReturn>(),
      ...auditActorColumns<PurchaseReturn>(userNameById),
      ...actionsColumn<PurchaseReturn>(showActions, (row) => {
