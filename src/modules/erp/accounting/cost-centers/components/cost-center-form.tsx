...
import {
  useCreateCostCenter,
  useUpdateCostCenter,
} from "@/modules/erp/accounting/cost-centers/mutations";
import {
...
function toFormValues(costCenter: CostCenter | null): CostCenterFormValues {
  return {
...
function toCreateRequest(values: CostCenterFormValues): CostCenterCreateRequest {
...
export function CostCenterForm({
  ...
  const form = useForm<CostCenterFormValues>({
    ...
    values: toFormValues(costCenter),
  });
  ...
  async function onSubmit(values: CostCenterFormValues) {
    ...
    try {
      if (costCenter) {
        ...
        const created = await createCostCenter.mutateAsync(payload);
  ...
  const pending = createCostCenter.isPending || updateCostCenter.isPending;
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <Input placeholder="Operations" disabled={disabled} {...field} />
                </FormControl>
                <FormMessage />
              </FormItem>
            )}
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <Input placeholder="OPS" disabled={disabled} {...field} />
                </FormControl>
                <FormMessage />
              </FormItem>
            )}
