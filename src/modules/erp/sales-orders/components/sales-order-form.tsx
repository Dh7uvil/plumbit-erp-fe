...
function toLineInput(line: SalesOrderLineFormValues): SalesOrderLineInput {
  return {
...
function toFormValues(
  ...
  return {
    ...
    branch_id: salesOrder?.branch_id ?? OPTIONAL_SELECT_NONE,
    warehouse_id: salesOrder?.warehouse_id ?? OPTIONAL_SELECT_NONE,
    ...
    currency_id: salesOrder?.currency_id ?? defaultCurrencyId ?? OPTIONAL_SELECT_NONE,
    ...
    salesperson_id: salesOrder?.salesperson_id ?? OPTIONAL_SELECT_NONE,
    notes: salesOrder?.notes ?? "",
    terms_and_conditions: salesOrder?.terms_and_conditions ?? "",
    terms_template_id: OPTIONAL_SELECT_NONE,
    discount_type: salesOrder?.discount_type ?? OPTIONAL_SELECT_NONE,
    discount_value: salesOrder?.discount_value ?? "",
    shipping_amount: salesOrder?.shipping_amount ?? "0",
    adjustment_amount: salesOrder?.adjustment_amount ?? "0",
    place_of_supply: salesOrder?.place_of_supply ?? OPTIONAL_SELECT_NONE,
    customer_trn: salesOrder?.customer_trn ?? "",
...
function toCreateRequest(values: SalesOrderFormValues): SalesOrderCreateRequest {
  return {
...
function toUpdateRequest(values: SalesOrderFormValues): SalesOrderUpdateRequest {
...
export function SalesOrderForm({
  ...
  const form = useForm<SalesOrderFormValues>({
    ...
    defaultValues: toFormValues(salesOrder, baseCurrencyId),
    values: salesOrder ? toFormValues(salesOrder, baseCurrencyId) : undefined,
  });
  ...
  const composeQuery = useSalesOrderComposeDefaults(isEdit ? null : selectedCustomerId);
  ...
  async function onSubmit(values: SalesOrderFormValues) {
    ...
    try {
      if (salesOrder) {
        ...
        const created = await createSalesOrder.mutateAsync(toCreateRequest(values));
  ...
  const pending = createSalesOrder.isPending || updateSalesOrder.isPending;
